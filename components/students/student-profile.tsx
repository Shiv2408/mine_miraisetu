...
function EditableField({ label, value, onSave }: EditableFieldProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Keep isEditing true on failure to allow user to retry
    } finally {
      setIsSaving(false);
    }
  };
...
export function StudentProfile({ studentId }: StudentProfileProps) {
  ...
  const handleUpdateInternalRef = async (
    ...
    const originalRefs = useStudentProfileStore.getState().internalRefs;
    useStudentProfileStore.setState((state) => {
      const newRefs = JSON.parse(JSON.stringify(state.internalRefs));
      const refType = type === "card" ? "cards" : "enos";
      let ref = newRefs[refType]?.find((r: any) => r.slot_number === slotNumber);
      if (ref) {
        ref.raw_value = value;
      } else {
        if (!newRefs[refType]) newRefs[refType] = [];
        newRefs[refType].push({ slot_number: slotNumber, raw_value: value });
      }
      return { internalRefs: newRefs };
    });
    ...
    try {
      const response = await fetch(`/api/students/${studentId}/internal-refs`, {
        ...
        body: JSON.stringify({
          refs: [{ ref_group: type, slot_number: slotNumber, raw_value: value }],
        }),
      });
      if (!response.ok) throw new Error("Failed to update internal ref");
    } catch (error) {
      useStudentProfileStore.setState({ internalRefs: originalRefs });
  ...
  const handleUpdateContact = async (contactType: string, value: string) => {
    const originalContact = useStudentProfileStore.getState().contact;
    useStudentProfileStore.setState((state) => {
      const newContact = JSON.parse(JSON.stringify(state.contact));
      let contactItem = newContact.contacts?.find(
        (c: any) => c.contact_type === contactType
      );
      if (contactItem) {
        contactItem.value_raw = value;
      } else {
        if (!newContact.contacts) newContact.contacts = [];
        newContact.contacts.push({ contact_type: contactType, value_raw: value });
      }
    ...
    try {
      const response = await fetch(`/api/students/${studentId}/contact`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          contacts: [{ contact_type: contactType, value_raw: value }],
        }),
      });
      if (!response.ok) throw new Error("Failed to update contact");
    } catch (error) {
      useStudentProfileStore.setState({ contact: originalContact });
  ...
  const handleAddNote = async (noteText: string) => {
    try {
      ...
      // For additions, reloading is the simplest way to get the new server-generated ID
      loadNotes(studentId);
    } catch (error) {
  ...
  const handleUpdateProfile = async (field: string, value: string) => {
    const originalOverview = useStudentProfileStore.getState().overview;
    useStudentProfileStore.setState((state) => {
      const newOverview = JSON.parse(JSON.stringify(state.overview));
      if (!newOverview.profile) newOverview.profile = {};
      newOverview.profile[field] = value;
      return { overview: newOverview };
    });
    ...
    try {
      ...
      useStudentProfileStore.setState({ overview: originalOverview });
  ...
  const handleUpdateEnrollment = async (field: string, value: string) => {
    ...
    useStudentProfileStore.setState((state) => {
      const newOverview = JSON.parse(JSON.stringify(state.overview));
      if (!newOverview.current_enrollment) newOverview.current_enrollment = {};
      newOverview.current_enrollment[field] = value;
      return { overview: newOverview };
    });
  ...
  const handleUpdateDocument = async (
    ...
    const originalDocuments = useStudentProfileStore.getState().documents;
    useStudentProfileStore.setState((state) => {
      const newDocuments = JSON.parse(JSON.stringify(state.documents));
      const doc = newDocuments.documents?.find((d: any) => d.id === documentId);
      if (doc) {
        doc[field] = value;
      }
    ...
    try {
      ...
      useStudentProfileStore.setState({ documents: originalDocuments });
  ...
  const handleUpdateAcademic = async (field: string, value: string) => {
    const originalAcademic = useStudentProfileStore.getState().academic;
    // This is complex due to the dynamic field name, so a reload on success is simpler
    try {
      ...
      if (!response.ok) throw new Error("Failed to update academic info");
      loadAcademic(studentId); // Reloading is safest for this complex case
    } catch (error) {
      useStudentProfileStore.setState({ academic: originalAcademic });
  ...
  const handleUpdateAddress = async (
    ...
    useStudentProfileStore.setState((state) => {
      const newContact = JSON.parse(JSON.stringify(state.contact));
      if (!newContact.addresses) newContact.addresses = {};
      if (!newContact.addresses[addressType]) newContact.addresses[addressType] = {};
      newContact.addresses[addressType][field] = value;
      return { contact: newContact };
    });
    ...
    try {
      const response = await fetch(`/api/students/${studentId}/contact`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ addresses: { [addressType]: { [field]: value } } }),
      });
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {Array.from({ length: 4 }, (_, i) => {
                      ...
                      const cardData = internalRefs?.cards?.find(
                        (c) => c.slot_number === slotNumber
                      );
                      return (
                        <EditableField
                          key={`card_${slotNumber}`}
                          label={`Card ${slotNumber}`}
                          value={cardData?.raw_value || ""}
                          onSave={(value) =>
                            handleUpdateInternalRef("card", slotNumber, value)
                          }
                        />
                      );
                    })}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {Array.from({ length: 8 }, (_, i) => {
                      ...
                      const enoData = internalRefs?.enos?.find(
                        (e) => e.slot_number === slotNumber
                      );
                      return (
                        <EditableField
                          key={`eno_${slotNumber}`}
                          label={`ENO ${slotNumber}`}
                          value={enoData?.raw_value || ""}
                          onSave={(value) =>
                            handleUpdateInternalRef("eno", slotNumber, value)
                          }
                        />
                      );
                    })}
