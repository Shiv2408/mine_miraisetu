...
/* --- helper/enriched types to avoid `any` --- */
...
type EnrichedEnrollment = {
  ...
  academic_sessions?: EnrichedSession;
  session_id?: string;
};
...
export class EnrollmentService {
  ...
  async getCurrentEnrollment(
    ...
    const { data: enrichedDataRaw, error: enrichError } = await supabase
    ...
    if (!enrichedDataRaw) return null;

    // Get current year from progressions; handle possible error
    ...
    if (progressionError) {
      // If progression reading fails, surface the error
      throw progressionError;
    }

    // Strongly type the enriched data to avoid `any`
    const enrichedData = enrichedDataRaw as EnrichedEnrollment;
    const course = enrichedData.courses as EnrichedCourse;
    const college = course?.colleges as EnrichedCollege;
    const session = enrichedData.academic_sessions as EnrichedSession;
    ...
    return {
      ...
      session_id: enrichedData.session_id || enrichedData.academic_sessions?.id || "",
      ...
      entry_year: enrichedData.entry_year || 0,
   ...
   * - Use a safer updates type (key-value) instead of `any`.
   * - Return a typed success object for better ergonomics.
   *
   * NOTE: currently this is a stub — you can implement per-field table updates here.
   */
  async updateAcademicInfo(
    studentId: string,
    updates: Record<string, unknown>
  ): Promise<{ success: boolean; message: string }> {
    // For now, just log updates — implement actual updates to tables as needed.
    console.log("Academic updates for student:", studentId, updates);

    // Return a typed success object
