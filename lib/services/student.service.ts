...
export class StudentService {
  ...
  async getStudentBasicInfo(
    ...
    return data as StudentBasicInfo | null;
  }
  ...
  async getStudentAddresses(studentId: string): Promise<{
    ...
    const addresses: StudentAddress[] = (data as StudentAddress[]) || [];

    const permanent = addresses.find(
      (addr) => addr.addr_type === "permanent"
    );
  ...
  async upsertStudentContact(
    studentId: string,
    contactType: StudentContact["contact_type"],
    value: string
  ...
  async getStudentNotes(
    ...
    // Get total count (head: true + count: 'exact')
  ...
  async updateStudentBasicInfo(
    ...
    if (error) throw error;
    return data as StudentBasicInfo;
  }
  ...
  async updateInternalRefs(
    ...
    const validRefs = refs.filter(
      (ref) => (ref.raw_value ?? "").trim().length > 0
    );

    if (validRefs.length === 0) return [];

    const insertPayload = validRefs.map((ref) => ({
      student_id: studentId,
      ...ref,
    }));
    ...
    if (error) throw error;
    return (data as StudentInternalRef[]) || [];
  }
}
